            ...
            <div className='w-full max-w-6xl h-full mx-auto relative py-[clamp(10px,3vw,30px)] px-[5vw] -translate-y-[20vh] rounded-md sm:rounded-xl bg-white'>
               <header className='w-full grid grid-cols-1 md:grid-cols-[1fr_400px] md:gap-10 gap-5 mb-5'>
                  <div className='w-full h-full text-center md:text-left flex flex-col justify-center gap-4'>
                     ...
                     <h2 className='text-base text-muted-foreground leading-5'>Inilah barisan jiwa yang mampu menjaga karsa dan menenun tutur kata dengan paling bijak. Apakah namamu tercatat di antara mereka?</h2>
                  </div>

                  <div className='w-full h-[324px] grid grid-cols-3 gap-2 items-end'>
                  ...
                  {topUsers && topUsers.map((user, index) => (
                     <div key={index} className="w-full p-3 md:px-6 flex bg-muted/40 rounded-md md:rounded-2xl items-center gap-3 md:gap-7 hover:bg-muted/60">
                        ...
                        <div className="ml-auto flex gap-2 justify-center items-center bg-primary py-2 px-4 rounded-lg overflow-hidden shrink-0">
                           ...
                           <span className="text-secondary line-clamp-1">{user.score}</span>
                        </div>
                     </div>
                  ))}
...
const RankBar = ({
   ...
   return (
      <div className={`w-full 
         ...
         ${item.rank === 2 && "h-[85%]"}
         ${item.rank === 3 && "h-[70%]"}
         rounded-lg overflow-hidden relative flex flex-col justify-end items-center p-2 group`}
         >
            {/* background */}
            ...
            <h3 className="font-bold text-lg">{item ? item?.username : "-"}</h3>
            <h4 className="italic">{item ? item?.title : "-"}</h4>
            <div className="flex gap-2 justify-center items-center w-full bg-primary p-1 rounded-lg overflow-hidden shrink-0">
               <CircleStar size={20} color="#F2EAD3"/>
