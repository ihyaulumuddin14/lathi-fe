...
const AlertDialogIntercept = () => {
  ...
  const { open, onOpenChange, alertDialogType, setAlertDialogType } = useAlertDialogIntercept(
    useShallow((state) => ({
      open: state.open,
        ...
        <AlertDialogHeader className="relative">
              ...
              <div className="w-full flex flex-col gap-5 items-center">
                ...
                >
                  ...
                  <rect
                    ...
                    height="24"
                    fill="#3F2305"
                    mask="url(#SVGWqb65B7o)"
                  />
                </svg>
                <h1>Tautan verifikasi telah terkirim</h1>
              </div>
            )}
          </AlertDialogTitle>
          ...
          {alertDialogType !== "warning" && (
            <div
              ...
              onClick={() => {
                onOpenChange(false);
                if ( pathname.startsWith("/play") || params.get("verified") === "true" ) {
                  router.replace("/");
                }
              }}
            >
          ...
          <AlertDialogDescription
            className={
              alertDialogType === "warning" ? "text-center mb-10" : "mb-5"
            }
          >
            {alertDialogType === "login" && (
              <>
                Senang melihatmu kembali di Lathi. Yuk, masuk sebentar untuk
                melanjutkan/memulai Lakonmu.
              </>
            )}
            ...
            {alertDialogType === "warning" && (
              <>
                Sepertinya kamu belum terdaftar. Masuk dulu yuk, biar progres
                ceritamu tidak hilang terbawa angin.
              </>
            )}
        ...
        <AlertDialogFooter>
          {alertDialogType === "login" && (
            <p className="text-muted-foreground text-sm mx-auto cursor-pointer">
              Belum punya akun?{" "}
              <span
                className="opacity-100 hover:opacity-55 transition-opacity ease-in-out"
